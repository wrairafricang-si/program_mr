...
ccs <- ggplot(cervical_cancer_screening, aes(x = "", y = Count, fill = Outcome)) +
  ...
  theme(legend.text = element_text(size = 30)) # Change 12 to your desired size
...
```{r title = "CERVICAL CANCER SCREENING OUTCOMES"}
ccs
```
...
```{r title = "VIRAL LOAD COVERAGE AMONG MALES", fig.width=15, fig.height=6}

vlc_male
```

```{r title = "VIRAL LOAD COVERAGE AMONG FEMALES", fig.width=15, fig.height=6}
# windows (width = 20, height = 10)
vlc_fem
```
